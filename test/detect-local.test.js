...
test('finds port and profile on dsh command lines, both invocation shapes', () => {
  const ps = [
  ...
  assert.deepEqual(parseDshPorts(ps), [
    { pid: 123, port: 3080, profile: 'web' },
    { pid: 4567, port: 52094, profile: 'desktop' },
  ]);
});

test('a port without a profile flag still resolves, profile null', () => {
  const ps = ' 99 node /opt/homebrew/bin/dsh --host 127.0.0.1 --port 4000';
  assert.deepEqual(parseDshPorts(ps), [{ pid: 99, port: 4000, profile: null }]);
});
...
test('skips port 0, duplicates, and non-dsh processes', () => {
  ...
  assert.deepEqual(parseDshPorts(ps), [{ pid: 3, port: 3080, profile: 'a' }]);
});
